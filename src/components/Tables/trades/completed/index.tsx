...
export function CompletedTradeList({ className, onClick, onLoadStart, onLoadEnd }: ListProps) {
  const {user} = useSelector(selectAuthState);
  const [trades, setTrades] = useState<Trade[]>([])
  const getTrades = async () => {
    onLoadStart && onLoadStart();
  const acceptedTradeListURL = `${process.env.NEXT_PUBLIC_API_URL}${routes.trade}/accepted/${user?.id}` || ''
    try {
      const res = await axiosExtended.get(acceptedTradeListURL);
      onLoadEnd && onLoadEnd();
      if (res.status === 200) {
